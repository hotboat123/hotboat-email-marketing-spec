...
import { formatDate } from "@/components/ads/BookingsModal";

export function ConversionBookingsModal({
  level,
  id,
  name,
  dateFrom,
  ...
  const { data: bookings = [], isLoading } = useQuery<AdBooking[]>({
    queryKey: ["ad-conversion-bookings", level, id, dateFrom, dateTo],
    queryFn: () => adsApi.conversionBookings(level, id, dateFrom, dateTo).then((r) => r.data),
    staleTime: 60_000,
  });
      ...
      <div className="bg-white rounded-xl shadow-lg w-full max-w-lg max-h-[80vh] flex flex-col">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">Mismo criterio de "pagó" que Tráfico Web (pagos/payment_status).</p>
          </div>
